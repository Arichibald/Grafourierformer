...
from utils.process import process_fourier


class GraphFourierFormer(torch.nn.Module):
    ...
    def forward(self, x, pe, edge_index, edge_attr, batch, fourier):
        if self.pe:
            pe = self.pe_norm(pe)
        # process node
        ...
        # get the sph
        fourier = process_fourier(fourier, self.slope)

        # Conv & MPNN
        if self.mpnn == 'GIN':
            for conv in self.convs:
                x = conv(x, edge_index, fourier, batch)
        elif self.mpnn == 'GCN':
            for conv in self.convs:
                x = conv(x, edge_index, fourier, batch, edge_attr=edge_attr, pe=pe)
        elif self.mpnn == 'GINE':
            for conv in self.convs:
                x = conv(x, edge_index, fourier, batch, edge_attr=edge_attr)
