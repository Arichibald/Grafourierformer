...
from model.GraphFourierFormer import GraphFourierFormer
from optimizer.optimizer_schedule import get_scheduler
...
def train(loader, task):
    ...
    for data in tqdm(loader, desc="Training", leave=False):
        data = data.to(device)
        node_num = data.fourier.shape[-1]
        data.fourier = data.fourier.reshape(-1, node_num, node_num)
        optimizer.zero_grad()
        if data.edge_attr is None:
            data.edge_attr = data.edge_index.new_zeros(data.edge_index.shape[1])

        out = model(data.x, data.pe, data.edge_index, data.edge_attr, data.batch, data.fourier)
        ...
        if task == "multi_label":
            ...
            print('outoutout', out.squeeze(), out.squeeze().shape)
            print('data.ydata.y', data.y, data.y.shape)
...
if __name__ == '__main__':
    for i in range(10):
        ...
        model = GraphFourierFormer(args=args, node_dim=num_features, edge_dim=num_edge_features, num_tasks=num_tasks,
        ...
        if args.dataset in ['NCI1', 'IMDB-BINARY', 'COLLAB', 'PROTEINS', 'MUTAG', 'ZINC']:
